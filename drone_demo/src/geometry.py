...
def sort_vertexes(polygon: List[Point]) -> List[Point] or None:
    ...
    result.append(polygon[ind])  # Добавляем найденную точку в результат
    del polygon[ind]  # Удаляем ее из исходного списка

    # Создаем список пар [угол наклона, вектор относительно минимальной точки]
    a = [[None, [i.x - result[0].x, i.y - result[0].y]] for i in polygon]

    # Считаем углы наклона относительно минимальной точки
    for i, p in enumerate(a):
        if p[1][0] != 0:
            p[0] = p[1][1] / p[1][0]  # tg(alpha)
        else:
            p[0] = float('inf')  # Вертикальная линия

    a.sort()  # Сортируем по углу наклона

    # Восстанавливаем координаты точек и добавляем в результат
    for k, p in a:
